...
/**
 * Created by JackyZ on 2017/8/29.
 * http://www.jianshu.com/p/9d034af56bea
 */
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                getParent().requestDisallowInterceptTouchEvent(true);//告诉上级不要拦截
                startX = ev.getX();//得到当前视图事件发生的x坐标点
                startY = ev.getY();//得到当前视图事件发生的y坐标点
                break;
            case MotionEvent.ACTION_MOVE:
                ...
                //判断事件的滑动方向
                if (Math.abs(distenceX) > Math.abs(distenceY) && Math.abs(distenceX) >= 8) {
                    //如果是水平滑动
                    getParent().requestDisallowInterceptTouchEvent(true);//告诉上级不要拦截
                } else {
