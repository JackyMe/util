...
//                    "lineHeight: " + (bottom - top));
     ...
     * @return
     */
    public static Spanned formatDuringPink(long mss) {
        long days = mss / (1000 * 60 * 60 * 24);
        long hours = (mss % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60);
        long minutes = (mss % (1000 * 60 * 60)) / (1000 * 60);
        String original = "<font size=\"3\" color=\"#FF5C60\">%02d</font>:<font size=\"18\" color=\"#FF5C60\">%02d</font>:<font size=\"18\" color=\"#FF5C60\">%02d</font>";
        System.out.println("original: " + String.format(original, days, hours, minutes));
        return Html.fromHtml(String.format(original, days, hours, minutes));
    }
     ...
     *
     * @param head       头部
     * @param content    中间改变部分
     * @param tail       尾巴
     * @param color      颜色
     * @param proportion 放大倍数
     * @return
     */
    public static Spanned formatPinkText(String head, String content, String tail, int color, float proportion) {  //中间部分变为粉色--并且字体变大
        SpannableString spannableString = new SpannableString(head + content + tail);
        try {
            RelativeSizeSpan sizeSpan01 = new RelativeSizeSpan(proportion);
            spannableString.setSpan(sizeSpan01, head.length(), head.length() + content.length(), Spanned.SPAN_INCLUSIVE_EXCLUSIVE); //设置中间变大
            ForegroundColorSpan colorSpan = new ForegroundColorSpan(color);
//            ForegroundColorSpan colorSpan = new ForegroundColorSpan(Color.parseColor("#000000"));
            spannableString.setSpan(colorSpan, head.length(), head.length() + content.length(), Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
        } catch (Exception e) {
            e.printStackTrace();
        }
